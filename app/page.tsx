    ...
    <main className="min-h-screen bg-black text-white flex flex-col items-center justify-center px-6 py-12 relative overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        {Array.from({ length: 30 }).map((_, i) => (
          <div
            key={i}
            className="sparkle w-2 h-2 rounded-full bg-pink-400 opacity-70"
            style={{
              top: `${Math.random() * 100}%`,
      ...
      {/* HERO SECTION */}
      <h1 className="text-5xl md:text-7xl font-extrabold text-pink-500 neon-glow text-center z-10">
        Pink Cheese
      </h1>

      <p className="mt-6 text-lg md:text-xl text-gray-300 text-center max-w-xl z-10">
        Send money. Spark confidence.
      </p>

      <button className="mt-10 px-8 py-4 bg-pink-600 rounded-full text-lg font-semibold hover:bg-pink-500 transition-all neon-button z-10">
        💖 Send Pink
      </button>

      {/* FLOATING APP CARD */}
      <div className="mt-16 w-full max-w-sm p-6 bg-black/80 backdrop-blur-md rounded-3xl shadow-[0_0_40px_#ff1493] border border-pink-500 text-center text-white z-10">
